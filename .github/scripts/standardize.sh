...
##############################################################################
# standardize.sh — Chip away at RuboCop, stree, and Prettier violations.
#
...
BRANCH_PREFIX="${BRANCH_PREFIX:-standardization}"
...
if [ ${#PR_SECTIONS[@]} -eq 0 ]; then
...

  {
    ...
    echo "---"
    echo "Existing CI gates merging. Generated by standardization workflow."
  } > "$PR_BODY_FILE"
